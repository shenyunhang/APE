...
def get_equivalent_boxes(all_boxes, iou_threshold=0.95):
    ...
    xy_boxes = box_xywh_to_xyxy(torch.as_tensor(all_boxes, dtype=torch.float))
...
def convert(
    subset: str, flickr_path: Path, output_path: Path, merge_ground_truth: bool, next_img_id: int = 1, next_id: int = 1
):
    ...
    for img_id in tqdm(ids):
        ...
        image = read_image(output_path / "flickr30k-images" / annotation["filename"], format="BGR")
        ...
        for sent_id, sent in enumerate(sents):
            ...
            for equiv in equivalences.values():
                ...
                phrase = " ".join([sentence[sp[0]:sp[1]] for sp in token_spans])
    ...
    if merge_ground_truth:
        filename = f"flickr30k_mergedGT_{subset}.json"
    else:
        filename = f"flickr30k_separateGT_{subset}.json"
